    <?php
    // importar archivos necesarios
    require '../server/con.php';
    require '../utils/template.php';
    require '../utils/querys.php';
    require '../utils/rango.php';

    header('Content-Type: application/json');

    $ejecutivoMap = [];
    $listaEjecutivos = [];

    // 1. Fetch all "ejecutivos" data for mapping
    $sql_executives = "SELECT * FROM ejecutivo";
    $raw_executives = query($sql_executives, $connection);

    if (is_array($raw_executives)) {
        foreach ($raw_executives as $executive) {
            $nombre = isset($executive['nom_eje']) ? $executive['nom_eje'] : "";
            $id = isset($executive['id_eje']) ? $executive['id_eje'] : "";
            $telefono = isset($executive['tel_eje']) ? $executive['tel_eje'] : "";

            $listaEjecutivos[] = $nombre;
            
            //Administrativo permisos = 1, otro = 0
            $permiso = (isset($executive['rol_eje']) && $executive['rol_eje'] === 'Administrativo') ? 1 : 0;
            if ($id != "") {
                $ejecutivoMap[$id] = [
                    'name' => $nombre,
                    'tel' => $telefono,
                    'tipo' => isset($executive['rol_eje']) ? $executive['rol_eje'] : "",
                    'permisos' => $permiso
                ];
            }
        }
    }

    // LOGICA DE PRACTICA 23
    // #######################################################
    $colores_clases = [
        "CITA AGENDADA" => "#ff9900a2",
        "INVASIÓN DE CICLO" => "#ffff0083",
        "CITA REAGENDADA" => "#9b27b077",
        "CITA NO ATENDIDA" => "#ff666677",
        "PAGO ESPERADO" => "#ff00ff7a",
        "PERDIDO POR PRECIO" => "#aabbcc79",
        "PERDIDO POR HORARIO" => "#3366996e",
        "REGISTRO" => "#00ffff81",
        "NO LE INTERESA" => "#cc00006e",
        "ASESORÍA REALIZADA" => "#00ff007c"
    ];

    // Definimos la lógica de colores
    $colores = [
        "CITA AGENDADA" => "status-agendada",
        "INVASIÓN DE CICLO" => "status-invasion",
        "CITA REAGENDADA" => "status-reagendada",
        "CITA NO ATENDIDA" => "status-no-atendida",
        "PAGO ESPERADO" => "status-pago-esperado",
        "PERDIDO POR PRECIO" => "status-perdido-precio",
        "PERDIDO POR HORARIO" => "status-perdido-horario",
        "REGISTRO" => "status-registro",
        "NO LE INTERESA" => "status-no-interesa",
        "ASESORÍA REALIZADA" => "status-asesoria"
    ];

    $soloNombres = array_keys($colores);
    // #######################################################

    // LOGICA PARA 24
    // #######################################################
    $colores_efectividad = [
        "CITA EFECTIVA" => "#FFC0CB",
        "CITA NO EFECTIVA" => "#FF6666"
    ];

    $clases_efectividad = [
        "CITA EFECTIVA" => "efectividad-efectiva",
        "CITA NO EFECTIVA" => "efectividad-no-efectiva"
    ];

    $soloNombresEfect = array_keys($colores_efectividad);
    // #######################################################

    $action = isset($_REQUEST['action']) ? $_REQUEST['action'] : '';

    // --- SCHEMA DEFINITION ---
    $schema = [
        ['data' => 'rango_calc', 'title' => 'RANGO', 'type' => 'text', 'readOnly' => true],
        ['data' => 'id_cita', 'title' => 'ID', 'type' => 'text', 'readOnly' => true],
        ['data' => 'date_cita', 'title' => 'FECHA', 'type' => 'date', 'dateFormat' => 'YYYY-MM-DD', 'correctFormat' => true],
        ['data' => 'hora_cit', 'title' => 'HORA', 'type' => 'time', 'timeFormat' => 'HH:mm:ss', 'correctFormat' => true],
        ['data' => 'nom_cita', 'title' => 'NOMBRE', 'type' => 'text'],
        ['data' => 'tel_eje', 'title' => 'TELÉFONO', 'type' => 'text', 'readOnly' => true],
        ['data' => 'nom_eje', 'title' => 'EJECUTIVO', 'type' => 'dropdown', 'source' => $listaEjecutivos],
        ['data' => 'est_cit', 'title' => 'ESTADO', 'type' => 'dropdown', 'source'=> $soloNombres],
        ['data' => 'efe_cit', 'title' => 'EFECTIVIDAD', 'type' => 'dropdown', 'source' => $soloNombresEfect]
    ];

    // --- HELPER: DATE FILTER CLAUSE ---
    function getDateClause($conn)
    {
        $start = isset($_REQUEST['startDate']) ? mysqli_real_escape_string($conn, $_REQUEST['startDate']) : '';
        $end = isset($_REQUEST['endDate']) ? mysqli_real_escape_string($conn, $_REQUEST['endDate']) : '';

        $clause = "";
        if (!empty($start) && !empty($end)) {
            $clause = " AND date_cita BETWEEN '$start' AND '$end' ";
        } elseif (!empty($start)) {
            $clause = " AND date_cita >= '$start' ";
        } elseif (!empty($end)) {
            $clause = " AND date_cita <= '$end' ";
        }
        return $clause;
    }


    switch ($action) {
        case 'modificar':
            $campo = escape($_POST['campo'], $connection);
            $valor = escape($_POST['valor'], $connection);
            $id_cit = escape($_POST['id_cit'], $connection);
            $sql = "UPDATE cita SET $campo = '$valor' WHERE id_cita = '$id_cit'";
            echo ejecutarConsulta($sql, $connection);
            break;

        case "eliminar":
            $id_cita = escape($_POST['id_cita'], $connection);
            $sql = "UPDATE cita SET is_active = 0, deleted_at = NOW() WHERE id_cita = '$id_cita'";  
            echo ejecutarConsulta($sql, $connection);
            break;

        case "agregar_vacio":
            $sql = "INSERT INTO cita (nom_cita, id_eje2) VALUES ('', NULL)";
            echo ejecutarConsulta($sql, $connection);
            break;

        case "obtener_modificado":
            $tipoEjecutivo = isset($_REQUEST["tipo"]) ?  mysqli_real_escape_string($connection, $_REQUEST['tipo']) : '';
            $raw_id = isset($_REQUEST['id_arbol']) ? str_replace(array("'", '"'), "", $_REQUEST['id_arbol']) : 0;
            $root_id = (int) $raw_id;

            $family_ids = [$root_id];
            $childrenMap = [];
            if (is_array($raw_executives)) {
                foreach ($raw_executives as $ex) {
                    $pid = isset($ex['id_padre']) ? $ex['id_padre'] : null;
                    $cid = $ex['id_eje'];
                    if ($pid !== null)
                        $childrenMap[$pid][] = $cid;
                }
            }

            $queue = [$root_id];
            while (!empty($queue)) {
                $current_parent = array_shift($queue);
                if (isset($childrenMap[$current_parent])) {
                    foreach ($childrenMap[$current_parent] as $child_id) {
                        $family_ids[] = $child_id;
                        $queue[] = $child_id;
                    }
                }
            }
            $ids_string = implode(',', $family_ids);
            if (empty($ids_string))
                $ids_string = "0";

            $dateSql = getDateClause($connection);

            $sql = '';

            if($tipoEjecutivo !=''){
            $sql = "SELECT *
                    FROM citaEjecutivo
                    WHERE rol_eje = '$tipoEjecutivo'
                    AND is_active = 1 
                    AND id_eje2 IN ($ids_string)
                    $dateSql
                    ORDER BY (hora_cit IS NULL), hora_cit";
            }else{
            $sql = "SELECT *
                    FROM citaEjecutivo 
                    WHERE is_active = 1 
                    AND id_eje2 IN ($ids_string)
                    $dateSql
                    ORDER BY (hora_cit IS NULL), hora_cit";
            }

            $raw_data = query($sql, $connection);
            $conteo_arbol = contarArbol($root_id);
            $conteo_arbol_efectividad = contarArbolEfectividad($root_id);
            outputData($raw_data, $schema, $ejecutivoMap, $colores, NULL, $conteo_arbol, NULL, NULL, $conteo_arbol_efectividad);
            break;
        case "obtener_eliminada":
            $sql = "SELECT *
                    FROM citaEjecutivo2
                    WHERE is_active = 0 
                    ORDER BY deleted_at DESC, (hora_cit IS NULL), hora_cit";

            $raw_data = query($sql, $connection);
            outputData($raw_data, $schema, $ejecutivoMap, $colores);
        break;
        case "restaurar":
            $id_cita = escape($_POST['id_cita'], $connection);
            $sql = "UPDATE cita SET is_active = 1, deleted_at = NULL WHERE id_cita = '$id_cita'";  
            echo ejecutarConsulta($sql, $connection);
        break;
        case "obtener_eje":
            $tipoEjecutivo = isset($_REQUEST["tipo"]) ?  mysqli_real_escape_string($connection, $_REQUEST['tipo']) : '';
            $id_eje_query = escape($_REQUEST['id_eje'], $connection);

            $dateSql = getDateClause($connection);

            $sql = '';

            if($tipoEjecutivo !=''){
                $sql = "SELECT *
                    FROM citaEjecutivo
                    WHERE is_active = 1 
                    AND id_eje2 = $id_eje_query
                    AND rol_eje = '$tipoEjecutivo'
                    $dateSql
                    ORDER BY (hora_cit IS NULL), hora_cit";

            }else{
                $sql = "SELECT *
                    FROM citaEjecutivo
                    WHERE is_active = 1 
                    AND id_eje2 = $id_eje_query
                    $dateSql
                    ORDER BY (hora_cit IS NULL), hora_cit";

            }

            $raw_data = query($sql, $connection);
            $countEstados = contarEje($id_eje_query);
            $countEfectividad = contarEjeEfectividad($id_eje_query);
            outputData($raw_data, $schema, $ejecutivoMap,$colores, $countEstados, NULL, $countEfectividad, $clases_efectividad);
            break;

            case "obtener_by_plantel":
            $raw_id = isset($_REQUEST['id_arbol']) ? str_replace(array("'", '"'), "", $_REQUEST['id_arbol']) : '';
            $id_arbol = mysqli_real_escape_string($connection, $raw_id);
            $tipoEjecutivo = isset($_REQUEST["tipo"]) ?  mysqli_real_escape_string($connection, $_REQUEST['tipo']) : '';


            $dateSql = getDateClause($connection);
            
            $sql = '';
            if($tipoEjecutivo !=''){
                $sql = "SELECT *
                        FROM citaEjecutivo
                        WHERE is_active = 1 
                        AND id_pla = '$id_arbol' 
                        AND rol_eje = '$tipoEjecutivo'
                        $dateSql
                        ORDER BY (hora_cit IS NULL), hora_cit";
            }else{
                $sql = "SELECT *
                        FROM citaEjecutivo
                        WHERE is_active = 1 
                        AND id_pla = '$id_arbol' 
                        $dateSql
                        ORDER BY (hora_cit IS NULL), hora_cit";
            }
        
            // 1. Consulta simplificada usando la nueva columna id_pla en cita

            $raw_data = query($sql, $connection);


            // 2. Obtener conteos específicos para el plantel
            $countEstados = contarPlantel($id_arbol);
            $countEfectividad = contarPlantelEfectividad($id_arbol);

            // 3. Salida de datos con todos los parámetros de efectividad y estados
            outputData($raw_data, $schema, $ejecutivoMap, $colores, $countEstados, NULL, $countEfectividad, $clases_efectividad);
            break;
    }

    function outputData($raw_data, 
                        $schema, 
                        $ejecutivoMap, 
                        $colores, 
                        $estados = NULL, 
                        $arbol = NULL, 
                        $countEfectividad=NULL, 
                        $coloresEfectividad = NULL,
                        $conteo_arbol_efectividad = NULL)
    {
        $processedData = [];
        if (is_array($raw_data)) {
            foreach ($raw_data as $row) {
                $id_eje_actual = $row['id_eje2'];
                $tel_eje_val = isset($ejecutivoMap[$id_eje_actual]) ? $ejecutivoMap[$id_eje_actual]['tel'] : "";
                $nom_eje_val = isset($ejecutivoMap[$id_eje_actual]) ? $ejecutivoMap[$id_eje_actual]['name'] : "";

                $processedData[] = [
                    'rango_calc' => "",
                    'id_cita' => $row['id_cita'],
                    'date_cita' => $row['date_cita'],
                    'hora_cit' => $row['hora_cit'],
                    'nom_cita' => $row['nom_cita'],
                    'tel_eje' => $tel_eje_val,
                    'nom_eje' => $nom_eje_val,
                    'est_cit' => $row['est_cit'],
                    'efe_cit' => $row['efe_cit']
                ];
            }
        }
        echo json_encode([
            'success' => true,
            'schema' => $schema,
            'data' => $processedData,
            'ejecutivoMap' => $ejecutivoMap,
            'coloresMap' => $colores,
            'count_estado' => $estados,
            'conteo_arbol' => $arbol,
            'conteo_efectividad' => $countEfectividad,
            'coloresMapEfectividad' => $coloresEfectividad,
            'conteo_arbol_efectividad' => $conteo_arbol_efectividad
        ]);
    }

    function contarEje($id_ejecutivo)
    {
        global $connection, $colores_clases;

        $id = mysqli_real_escape_string($connection, $id_ejecutivo);
        $dateSql = getDateClause($connection);

        $sql = "SELECT est_cit, COUNT(*) as total 
                FROM cita 
                WHERE is_active = 1 
                AND id_eje2 = '$id' 
                $dateSql 
                GROUP BY est_cit";

        $result = query($sql, $connection);
        $stats = [];

        foreach ($colores_clases as $name => $hex) {
            $stats[$name] = [
                'name' => $name,
                'color' => $hex,
                'count' => 0
            ];
        }

        if (is_array($result)) {
            foreach ($result as $row) {
                $statusName = $row['est_cit'];
                $count = (int) $row['total'];

                $color = isset($colores_clases[$statusName]) ? $colores_clases[$statusName] : '#000000';

                $stats[$statusName] = [
                    'name' => $statusName,
                    'color' => $color,
                    'count' => $count
                ];
            }
        }

        return $stats;
    }

    function contarArbol($id_arbol)
    {
        global $connection, $colores_clases, $raw_executives;

        $root_id = (int) $id_arbol;

        $family_ids = [$root_id];
        $childrenMap = [];

        if (is_array($raw_executives)) {
            foreach ($raw_executives as $ex) {
                $pid = isset($ex['id_padre']) ? $ex['id_padre'] : null;
                $cid = $ex['id_eje'];
                if ($pid !== null) {
                    $childrenMap[$pid][] = $cid;
                }
            }
        }

        $queue = [$root_id];
        while (!empty($queue)) {
            $current_parent = array_shift($queue);
            if (isset($childrenMap[$current_parent])) {
                foreach ($childrenMap[$current_parent] as $child_id) {
                    $family_ids[] = $child_id;
                    $queue[] = $child_id;
                }
            }
        }

        $ids_string = implode(',', $family_ids);
        if (empty($ids_string))
            $ids_string = "0";

        $dateSql = getDateClause($connection);

        $sql = "SELECT est_cit, COUNT(*) as total 
                FROM cita 
                WHERE is_active = 1 
                AND id_eje2 IN ($ids_string) 
                $dateSql 
                GROUP BY est_cit";

        $result = query($sql, $connection);
        $stats = [];

        foreach ($colores_clases as $name => $hex) {
            $stats[$name] = [
                'name' => $name,
                'color' => $hex,
                'count' => 0
            ];
        }

        if (is_array($result)) {
            foreach ($result as $row) {
                $statusName = $row['est_cit'];
                $count = (int) $row['total'];

                $color = isset($colores_clases[$statusName]) ? $colores_clases[$statusName] : '#000000';

                $stats[$statusName] = [
                    'name' => $statusName,
                    'color' => $color,
                    'count' => $count
                ];
            }
        }

        return $stats;
    }

    function contarEjeEfectividad($id_ejecutivo)
    {
        global $connection, $colores_efectividad;

        $id = mysqli_real_escape_string($connection, $id_ejecutivo);
        $dateSql = getDateClause($connection);

        $sql = "SELECT efe_cit, COUNT(*) as total 
                FROM cita 
                WHERE is_active = 1 
                AND id_eje2 = '$id' 
                $dateSql 
                GROUP BY efe_cit";

        $result = query($sql, $connection);
        $stats = [];

        foreach ($colores_efectividad as $name => $hex) {
            $stats[$name] = [
                'name' => $name,
                'color' => $hex,
                'count' => 0
            ];
        }

        if (is_array($result)) {
            foreach ($result as $row) {
                $statusName = $row['efe_cit'];
                $count = (int) $row['total'];

                $color = isset($colores_efectividad[$statusName]) ? $colores_efectividad[$statusName] : '#000000';

                $stats[$statusName] = [
                    'name' => $statusName,
                    'color' => $color,
                    'count' => $count
                ];
            }
        }

        return $stats;
    }
    function contarArbolEfectividad($id_arbol)
    {
        global $connection, $colores_efectividad, $raw_executives;

        $root_id = (int) $id_arbol;

        $family_ids = [$root_id];
        $childrenMap = [];

        if (is_array($raw_executives)) {
            foreach ($raw_executives as $ex) {
                $pid = isset($ex['id_padre']) ? $ex['id_padre'] : null;
                $cid = $ex['id_eje'];
                if ($pid !== null) {
                    $childrenMap[$pid][] = $cid;
                }
            }
        }

        $queue = [$root_id];
        while (!empty($queue)) {
            $current_parent = array_shift($queue);
            if (isset($childrenMap[$current_parent])) {
                foreach ($childrenMap[$current_parent] as $child_id) {
                    $family_ids[] = $child_id;
                    $queue[] = $child_id;
                }
            }
        }

        $ids_string = implode(',', $family_ids);
        if (empty($ids_string))
            $ids_string = "0";

        $dateSql = getDateClause($connection);

        $sql = "SELECT efe_cit, COUNT(*) as total 
                FROM cita 
                WHERE is_active = 1 
                AND id_eje2 IN ($ids_string) 
                $dateSql 
                GROUP BY efe_cit";

        $result = query($sql, $connection);
        $stats = [];

        foreach ($colores_efectividad as $name => $hex) {
            $stats[$name] = [
                'name' => $name,
                'color' => $hex,
                'count' => 0
            ];
        }

        if (is_array($result)) {
            foreach ($result as $row) {
                $statusName = $row['efe_cit'];
                $count = (int) $row['total'];

                $color = isset($colores_efectividad[$statusName]) ? $colores_efectividad[$statusName] : '#000000';

                $stats[$statusName] = [
                    'name' => $statusName,
                    'color' => $color,
                    'count' => $count
                ];
            }
        }

        return $stats;
    }

    function contarPlantel($id_plantel)
    {
        global $connection, $colores_clases;
        $id = mysqli_real_escape_string($connection, $id_plantel);
        $dateSql = getDateClause($connection);

        $sql = "SELECT est_cit, COUNT(*) as total 
                FROM cita 
                WHERE is_active = 1 AND id_pla = '$id' $dateSql 
                GROUP BY est_cit";

        $result = query($sql, $connection);
        $stats = [];
        foreach ($colores_clases as $name => $hex) {
            $stats[$name] = ['name' => $name, 'color' => $hex, 'count' => 0];
        }
        if (is_array($result)) {
            foreach ($result as $row) {
                $statusName = $row['est_cit'];
                $stats[$statusName]['count'] = (int) $row['total'];
            }
        }
        return $stats;
    }

    function contarPlantelEfectividad($id_plantel)
    {
        global $connection, $colores_efectividad;
        $id = mysqli_real_escape_string($connection, $id_plantel);
        $dateSql = getDateClause($connection);

        $sql = "SELECT efe_cit, COUNT(*) as total 
                FROM cita 
                WHERE is_active = 1 AND id_pla = '$id' $dateSql 
                GROUP BY efe_cit";

        $result = query($sql, $connection);
        $stats = [];
        foreach ($colores_efectividad as $name => $hex) {
            $stats[$name] = ['name' => $name, 'color' => $hex, 'count' => 0];
        }
        if (is_array($result)) {
            foreach ($result as $row) {
                $statusName = $row['efe_cit'];
                if (isset($stats[$statusName])) {
                    $stats[$statusName]['count'] = (int) $row['total'];
                }
            }
        }
        return $stats;
    }

    mysqli_close($connection);
    exit;

    ?>