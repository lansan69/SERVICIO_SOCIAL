...
switch ($action) {
    case 'obtener_arbol':
        ...
        // (We removed the global $sql3 block here to do it per-row below)
        ...
        if (is_array($raw_data)) {
            foreach ($raw_data as $row) {
                ...
                // 3. Count ONLY the extra assignments in planteles_ejecutivo for this specific ID
                ...
                // Total Buildings = 1 (The Main Plantel they belong to) + Extra Assignments
                $total_count = $extra_count;

                // --- GENERATE HTML ---
                $iconHtml = "";

                // Only show badge if they have at least 1 (which they always should)
                if ($total_count > 0) {
                    $iconHtml .= " <span style='color:#ccc; margin-left:5px'>|</span> ";

                    for ($i = 0; $i < $total_count; $i++) {
                        $iconHtml .= "<span class='building-badge'>domain</span>";
                    }
                }
