...
switch ($action) {
    ...
    case "obtener_by_plantel":
        ...
        // 2. MODIFIED SQL to use 'planteles_ejecutivo'
        // We join cita -> planteles_ejecutivo based on the executive ID (id_eje)
        // Then we filter by the specific Plantel UUID ($id_arbol)
        $sql = "SELECT DISTINCT 
                    ...
                    cita.hora_cit as hora_cit
                FROM cita
                INNER JOIN planteles_ejecutivo pe 
                    ON cita.id_eje2 = pe.id_eje
                WHERE cita.is_active = 1 
